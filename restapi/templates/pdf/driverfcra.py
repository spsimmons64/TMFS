...
def print_signature(pdf,driver):
    dco_set,dco_cnt = Database().query("SELECT * FROM documenttypes WHERE typecode='41' AND deleted IS NULL")            
    if dco_cnt:    
        sql = "SELECT * FROM driverdocuments WHERE driverid=%s AND documenttypeid=%s AND deleted IS NULL "
        sig_set,sig_cnt = Database().query(sql,(driver["recordid"],dco_set[0]["recordid"]))                        
        if sig_cnt:    
            esig_rec = Drivers().get_driver_esignature(driver["socialsecurity"],driver["birthdate"],False)
            img = Image.open(BytesIO(esig_rec))
            image_array = BytesIO()
            ...
            pdf.cell(w=1,text=format_date_time(sig_set[0]["driversignaturedate"],"human_date"))        
