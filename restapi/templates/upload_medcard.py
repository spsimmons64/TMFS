...
def send_message(driverid,link,email=""):    
    ...
    url = link
    hdr = f"""Upload Your Medical Certificate"""
    msg = f"""
            <tr><td><hr/></td></tr>
            <tr><td>
            <p>Hello {drv_rec["firstname"]} {drv_rec["lastname"]}:</p>
            <p>{acc_rec['companyname']} has requested that you upload a copy of your medical certificate. Please follow the link below 
               to securely upload:</p>
            <p style="font-weight:bold;text-align=center;"><a href="{link}" target="_blank">Upload My Medical Certificate</p>
            <tr><td>            
            ...
            <tr><td><hr/></td></tr>
    """
    email= SendEmail(smtp,from_list,to_list,bcc_list,f"Upload Your Medical Certificate To {acc_rec['companyname']}",hdr,msg,images,attachments=attachments)
    return(email.send())