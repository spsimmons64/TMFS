...
def send_message(id):    
    ...
    hdr = f"Password Changed"
    msg = f"""
            <tr><td><hr/></td></tr>
            <tr><td>
                <p>Hello {usr_rec["firstname"]} {usr_rec["lastname"]}:</p>
                <p>Your {ent_rec["companyname"]} password has successfully been changed for the account associated with the
                    email address {usr_rec["emailaddress"]}.</p>
                <p>If you didn't request this change, please ignore this email and contact our support team at 
                    <a href="mailto:{from_list[0]}" target="_blank">{from_list[0]}</a>
                    to report this incident.</p>
            <p>Sincerely</p>            
            </td></tr>
            <tr><td><img src="cid:logo" alt='Logo' /></td></tr>
            <tr><td><hr/></td></tr>
    """
    email= SendEmail(smtp,from_list,to_list,bcc_list,"Password Change",hdr,msg,images)
    return(email.send())    
