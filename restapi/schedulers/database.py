...
class Database():
    def __init__(self):
        self.server = config.DATABASE_URL
        self.catalog = config.DATABASE_CAT
        self.user = config.DATABASE_UID
        self.pwd = config.DATABASE_PWD
        self.port = config.DATABASE_PRT
        self.deactivate_field = "deactivated"

    def __write_log_error(self, errnum, errmsg, sql=""):
        log_file = config.DATABASE_LOG
        ...
        for col in columns:                        
            if col["name"] in record:
                ...
                if d_type in MYSQL_NUMERIC:                          
                    if record[col["name"]]==None or record[col["name"]]=="": 
                        record[col["name"]]=0
