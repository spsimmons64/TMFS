import pytz,base64,locale,os,string,secrets,shutil,io
...
def generate_signature(name,recordid,date,ipaddress):
    ...
    image = Image.new('RGBA', (image_width, image_height), (0,0,0,0))
    draw = ImageDraw.Draw(image)
    draw.text((3,3), name, fill=(51, 51, 51), font=name_font)    
    draw.text((name_width+20,20),recordid,fill=(51,51,51,51),font=font) 
    new_text = f"{date} / {ipaddress}"
    draw.text((name_width+20,36),new_text,fill=(51,51,51,51),font=font) 
    byte_stream = BytesIO()
