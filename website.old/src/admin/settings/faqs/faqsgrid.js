...
export const FAQSGrid = ({ assets, setPage }) => {    
    ...
    const columns = [{
        ...
        hdrStyle: {},
        sortable: 1,
        render: (ndx) => <div className="short" style={{ fontSize: "14px", fontWeight: 600 }}>{gridState.data[ndx].question}</div> 
        ...
        render: (ndx) =>
            <GridActionColumn>
                <GridAction title="Edit This Question" data-id={gridState.data[ndx].recordid} onClick={handleUpdate}>
    ...
    const handleUpdate = ({ target }) => {
        ...
        setPage({ page: 804, subpage: 1, entity: "", entityRecord: {}, record: rec || {} })
    }

    const handleSelected = ({ target }) => {
        let rec = gridState.data.find(r => r.recordid === target.getAttribute("data-key"))
        rec && setSelected({ index: rec.recordid, text: rec.answer })
    }
    ...
    useEffect(() => {
        if (gridState.data.length && selected.text == "") {
            setSelected({ index: gridState.data[0].recordid, text: gridState.data[0].answer })
        }
    }, [gridState.data])

    useEffect(() => { posData.status == 200 && updateGridData() }, [posData])

    useEffect(() => { updateGridData({ url: "faqs", sortcol: "position", sortdir: "asc" }) }, [])

    return (
        <PortalPlayGroundStyle>
            <PortalPlayGroundHeaderStyle>
                <PortalPlayGroundBreadCrumbStyle>TMFS Administration &gt; Frequently Asked Questions</PortalPlayGroundBreadCrumbStyle>
                <PortalPlayGroundPageTitleStyle>Frequently Asked Questions</PortalPlayGroundPageTitleStyle>
            </PortalPlayGroundHeaderStyle>
            <PortalPlaygroundScrollContainerStyle>
                <GridToolBox>
                    <div style={{ flex: 1, fontWeight: 600, fontSize: "18px" }}>Frequently Asked Questions Listing</div>
                    <div style={{ marginRight: "20px" }}>
                        <FormCheck style={{ color: "#737373" }} label="Include Deactivated" onChange={handleDeactivated} hideerror={1} />
                    </div>
                    <div style={{ width: "400px" }}><GridSearchInput placeholder="Search FAQs..." id="search-articles" callback={handleSearch} /></div>
                    <div style={{ marginLeft: "10px" }}>
                        <FormButton style={{ height: "34px" }} onClick={handleUpdate}>{`New FAQ`}</FormButton>
                    </div>
                </GridToolBox>
                <div style={{ display: "flex", width: "100%", flex: 1, }}>
                    <div style={{ display: "flex", flexFlow: "column", width: "600px", backgroundColor: "pink" }}>
                        <GridWrapper>
                            {gridState.busy
                                ? <GridLoader />
                    ...
                    <div style={{ flex: 1, border: "1px solid #808080", backgroundColor: "#e6f2a2" }}>
                        <ArticleViewer id="faq-viewer" value={selected.text} height="100%"></ArticleViewer>
                    </div>
                </div>
            </PortalPlaygroundScrollContainerStyle>
        </PortalPlayGroundStyle>
