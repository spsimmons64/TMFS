...
export const AccountsGrid = ({ status, setCounts, setPage }) => {
    ...
    useEffect(() => {
        if (gridState.status === 200) {
            ...
            setCounts({ linked: linked, pending: pending })
        }
    }, [gridState.data])

    return (<>
        <span style={{ fontSize: "21px", fontWeight: 700, margin: "19px 0px" }}>
            {status == "linked" ? "Linked Accounts" : "Pending Invitations"}
        </span>
        <div style={{ display: "flex", alignItems: "center", marginBottom: "20px" }}>
            <div style={{ flex: 1 }}>
                {status == "linked"
                    ? <span>Below is a list of accounts linked to <b>{globalState.consultant.companyname}</b>.</span>
                    : <span>Below is a list of any pending invitations from accounts.</span>
                }
            </div>
