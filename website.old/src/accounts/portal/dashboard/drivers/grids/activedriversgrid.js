...
align-items:center;
`
export const ActiveDriversGrid = () => {
    ...
    const columns = [{
        ...
        hdrAlign: "center",
        sortable: 1,
        render: (ndx) => <div className="short">{gridState.data[ndx].hired}</div>
    }, {
        id: "action",
        width: "8%",
        header: "Actions",
    ...
    useEffect(() => {
        if (globalState.account.recordid) {
            ...
            setGridUrl(`drivers?action=grid&parentid=${globalState.account.recordid}&route=active&entity=account`, false)
    ...
    return (<>
        {!driverForm
            ? <>
                <PortalPlayGroundPageTitleStyle><h1>Active Drivers</h1></PortalPlayGroundPageTitleStyle>
