...
align-items:center;
`
export const IncompleteDriversGrid = () => {
    ...
    const columns = [{
        ...
        hdrAlign: "center",
        sortable: 1,
        render: (ndx) => <div className="short">{gridState.data[ndx].appstep}</div>
    }]
    ...
    useEffect(() => {
        if (globalState.account.recordid) {
            ...
            setGridUrl(`drivers?action=grid&parentid=${globalState.account.recordid}&route=incomplete&entity=account`, false)
    ...
    return (
        <>
            {!driverForm
                ? <>
                    <PortalPlayGroundPageTitleStyle><h1>Incomplete Applications</h1></PortalPlayGroundPageTitleStyle>
