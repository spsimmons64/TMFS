...
`
export const SetupPage12 = ({ account }) => {
    ...
    const checkForSignature = () => {
        if (getValue("drv_signatures").length) {
            const rec = getValue("drv_signatures").find(r => r.sig_typecode == "13")
            setSignature(rec || {})
    ...
    const getPdfFile = async () => {
        let url = `${getApiUrl()}/fetchobj/account/adpolicy?id=${getValue("acc_recordid")}`
    ...
    return (<>
        <div style={{ fontSize: "24px", fontWeight: 700, padding: "10px 0px 10px 0px" }}>Alcohol & Drug Testing Policy</div>
        <div style={{ margin: "5px 0px 20px 0px", color: "#0A21C0" }}><b>Please review the company alcohol and drug testing policy below. Once finished,
            electronically sign to confirm receipt, understanding and agree. Upon completion of the online application, you will be
            emailed a copy of this policy. You may also click the <strong>Download</strong> button in the top right corner of the viewer to save a
            copy of the policy now.</b>
        </div>
