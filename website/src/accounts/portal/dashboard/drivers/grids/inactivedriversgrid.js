...
export const InactiveDriversGrid = () => {
    ...
    const columns = [{
        ...
        hdrAlign: "center",
        sortable: 1,
        render: (ndx) => <div className="short">{gridState.data[ndx].inactive}</div>
    }, {
        id: "action",
        width: "8%",
        header: "Actions",
    ...
    useEffect(() => {
        if (globalState.account.recordid) {
            ...
            setGridUrl(`drivers?action=grid&parentid=${globalState.account.recordid}&route=inactive&entity=account`, false)
    ...
    return (
        <>
            {!driverForm
                ? <>
                    <PortalPlayGroundPageTitleStyle><h1>Inactive Drivers</h1></PortalPlayGroundPageTitleStyle>
