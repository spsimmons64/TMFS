...
align-items:center;
`
export const PendingDriversGrid = () => {
    ...
    const columns = [{
        ...
        hdrAlign: "center",
        sortable: 1,
        render: (ndx) => <div className="short">{gridState.data[ndx].pending}</div>
    }, {
        id: "action",
        width: "8%",
        header: "Actions",
    ...
    useEffect(() => {
        if (globalState.account.recordid) {
            ...
            setGridUrl(`drivers?action=grid&parentid=${globalState.account.recordid}&route=pending&entity=account`, false)
    ...
    return (
        <>
            {!driverForm
                ? <>
                    <PortalPlayGroundPageTitleStyle><h1>Pending Drivers</h1></PortalPlayGroundPageTitleStyle>
